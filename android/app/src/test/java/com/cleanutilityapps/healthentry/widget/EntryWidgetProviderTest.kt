...
class EntryWidgetProviderTest {
    ...
    @Test fun firstWaterClickContainsPresetAndIsAcceptedExactlyOnce() {
        ...
        val next = requireNotNull(gate.open(click(widget).data?.host, false, false))
        assertNotEquals(id, next)
        assertNull(gate.consume(id))
        assertFalse(gate.close(id)) // Old callback cannot close the new entry.
        assertEquals("water", gate.consume(next))
    }
    ...
    @Test fun malformedOrRestoredDeliveryDoesNotPoisonTheNextLegitimateClick() {
        ...
        assertNull(gate.open(click(widget).data?.host, true, false))
        assertNull(gate.open(click(widget).data?.host, false, true))
        val id = requireNotNull(gate.open(click(widget).data?.host, false, false))
        assertEquals("water", gate.consume(id))
        assertTrue(gate.close(id))
        assertNotNull(gate.open(click(widget).data?.host, false, false))
    }
}
